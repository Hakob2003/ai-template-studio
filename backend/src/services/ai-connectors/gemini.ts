...
export class GeminiConnector extends BaseAIConnector {
  ...
  async generate(
    ...
    try {
      let modelId = this.config.modelId || 'imagen-3.0-generate-001';
      // Если пользователь случайно ввёл "Gemini" как Model ID
      if (modelId.toLowerCase() === 'gemini') {
      ...
      try {
        // Pollinations.ai может требовать оплату (402) с серверных IP.
        // Используем заглушку LoremFlickr, если генерация упала.
        const seed = Math.floor(Math.random() * 1000);
        const firstWord = prompt.split(' ')[0].replace(/[^a-zA-Z]/g, '') || 'art';
        const fallbackUrl = `https://loremflickr.com/1024/1024/${firstWord}?lock=${seed}`;
